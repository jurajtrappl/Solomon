const { rollExpressionLexer } = require('./lexer');
...
/* roll expression parser specifics */
...
const parse = (input) => {
    rollExpressionLexer.setInput(input);
    var tokens = [], token;
    while (token = rollExpressionLexer.lex()) tokens.push(token);
